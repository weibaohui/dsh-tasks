...
- **通知推送**：任务开始 / 完成 / 失败时，把结果推送到你绑定的 IM 聊天渠道（微信、飞书、钉钉、Telegram 等）——通过 [dsh-im](https://github.com/xmanrui/dsh-im) 等 IM 投递类插件完成，执行结论（agent 最终回复）可随完成通知一并推送；支持同时配置多个渠道，多渠道同时推送；未安装对应插件时通知自动不可用，不影响其他功能
